...
const toExpensePayload = form => ({
  ...
  creditCardId: form.creditCardId && form.creditCardId !== CASH_OPTION ? parseInt(form.creditCardId, 10) : null,
...
export default function ExpensesPage() {
  ...
  const submit = async e => {
    ...
    try {
      ...
      if (editingExpenseId) {
        ...
        setExpenses(current => current.map(expense => expense.id === editingExpenseId ? data : expense));
  ...
  const startEdit = expense => {
    ...
    setForm({
      ...
      creditCardId: expense.cardName ? (expense.creditCardId ? expense.creditCardId.toString() : CASH_OPTION) : '',
  ...
  const totalCashback = expenses.reduce((sum, expense) => sum + expense.cashbackEarned, 0);
        ...
        <div className="card">
          ...
          <form onSubmit={submit}>
            <div className="form-group">
              <label>Title</label>
              <input name="title" value={form.title} onChange={handle} required placeholder="e.g. Electricity Bill" />
            </div>
            ...
            <div className="form-group">
              ...
              <select name="creditCardId" value={form.creditCardId} onChange={handle}>
                <option value="">Select</option>
                <option value={CASH_OPTION}>Cash (0% cashback)</option>
                {cards.map(card => <option key={card.id} value={card.id}>{card.cardName} ...{card.last4Digits}</option>)}
              </select>
            </div>
            <div className="form-group">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div className="form-group">
                ...
                <select value={year} onChange={e => setYear(+e.target.value)}>
                  {[2023, 2024, 2025, 2026].map(value => <option key={value}>{value}</option>)}
                </select>
              </div>
            </div>
          </div>
          ...
          {selectedExpense && (
            <div className="card">
              ...
              <div style={{ display: 'grid', gap: 10, fontSize: '0.9rem' }}>
                ...
                <div><strong>Payment:</strong> {selectedExpense.cardName || 'Cash'}</div>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th>Payment</th><th>Amount</th><th>Cashback</th><th>Notes</th><th>View</th><th>Edit</th><th></th>
                  </tr>
                </thead>
                <tbody>
                  {expenses.map(expense => (
                    <tr key={expense.id}>
                      <td className="text-muted">{new Date(expense.date).toLocaleDateString()}</td>
                      <td style={{ fontWeight: 500 }}>{expense.title}</td>
                      <td><span className="badge badge-blue">{categoryIcon(expense.category)} {expense.category}</span></td>
                      <td className="text-muted">{expense.cardName || 'Cash'}</td>
                      <td style={{ fontWeight: 700 }}>${expense.amount.toFixed(2)}</td>
                      <td className="text-green">+${expense.cashbackEarned.toFixed(2)}</td>
                      <td>{expense.notes?.trim() ? <span className="badge badge-green">Has notes</span> : <span className="text-muted">-</span>}</td>
                      <td>
                        <button type="button" className="btn btn-sm" style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)' }} onClick={() => setSelectedExpense(expense)}>View</button>
                      </td>
                      <td>
                        <button type="button" className="btn btn-sm btn-primary" onClick={() => startEdit(expense)}>Edit</button>
                      </td>
                      <td>
                        <button className="btn btn-sm btn-danger" onClick={() => remove(expense.id)}>Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
